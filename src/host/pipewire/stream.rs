...
use crate::{traits::StreamTrait, InputCallbackInfo, SampleFormat, StreamConfig, StreamError};
use pipewire::{
    self as pw,
    context::ContextRc,
...
struct StreamData<D, E> {
    mainloop: MainLoopRc,
    listener: StreamListener<UserData<D, E>>,
    stream: StreamRc,
    context: ContextRc,
}

fn connect_input<D, E>(
    config: &StreamConfig,
    ...
    let listener = stream
        ...
        .process(|stream, user_data| match stream.dequeue_buffer() {
            ...
            Some(mut buffer) => {
                ...
                let n_samples = data.chunk().size() / user_data.samples_format.sample_size() as u32;
                ...
                let _data =
                    unsafe { Data::from_parts(data, n_samples as usize, user_data.samples_format) };
                //(user_data.data_callback)(&data,crate::InputCallbackInfo::new(Ins) )
            }
        })
        .register()?;
    let mut audio_info = pw::spa::param::audio::AudioInfoRaw::new();
    audio_info.set_format(samples_format.into());
