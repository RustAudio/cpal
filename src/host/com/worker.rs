...
#[derive(Debug, Clone)]
pub struct Handle(SyncSender<Request>);
...
impl Handle {
    pub fn new() -> Self {
        ...
        thread::spawn(move || {
            ...
            let _guard = com::InitGuard::new(COINIT_APARTMENTTHREADED)
    ...
    pub fn create_driver(&self, guid: GUID) -> Response {
        ...
        self.0
            .send((guid, ret_sender))
            .expect("the worker thread should never die prematurely");
        ret_receiver
            .recv()
            .expect("the worker thread should never die prematurely")
    }
