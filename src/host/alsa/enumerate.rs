...
// ALSA PCM device enumeration limits (SNDRV_PCM_DEVICES):
// - Without CONFIG_SND_DYNAMIC_MINORS (default): 8 devices per card max
// - With CONFIG_SND_DYNAMIC_MINORS enabled: 254 devices per card max
const MAX_DEVICE_INDEX: u32 = 254;
...
fn physical_devices() -> Vec<PhysicalDevice> {
    ...
    for card in alsa::card::Iter::new().filter_map(Result::ok) {
        ...
        // Device indices may be non-contiguous  (e.g., 0, 1, 3, 7), so we check all indices.
        for device_index in 0..MAX_DEVICE_INDEX {
